...
export async function runMatchStagingPreflightCli(
  argv: string[],
  io: PreflightCliIo = {
    cwd: process.cwd(),
    env: process.env,
  ...
  if (!webEnvPath || !orchestratorEnvPath) {
    throw new Error(
      "Usage: match:staging:preflight --web-env <path> --orchestrator-env <path> --backup-restore-verified 1 --retention-migration-verified 1 --provider-privacy-approved 1",
    );
  }
  ...
  const result = validateMatchStagingPreflight({
    ...
    retentionMigrationVerified: options.get("retention-migration-verified") === "1",
    providerPrivacyApproved: options.get("provider-privacy-approved") === "1",
  });
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  runMatchStagingPreflightCli(process.argv.slice(2))
    .then((result) => {
      process.stdout.write(result.stdout);
      if (result.exitCode !== 0) process.exitCode = result.exitCode;
    })
    .catch((error: unknown) => {
      console.error(error instanceof Error ? error.message : "Match staging preflight failed.");
      process.exitCode = 1;
    });
}
